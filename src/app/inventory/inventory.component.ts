...
export class InventoryComponent implements OnInit {
 ...
 public DataQuery_Store_Id: any=0;
  constructor(private authService: ApiService, private formBuilder: FormBuilder,private router: Router, private spinnerService: Ng4LoadingSpinnerService) {
    if (this.router.getCurrentNavigation().extras.state) {
      this.Overview_Store_Id = this.router.getCurrentNavigation().extras.state.Store_Id;
      // console.log("Inv_Dash",Store_Id);
...
DealerDropDown(){
  ...
  this.authService.AXELPostmethod('AXELData/GetAxelDealers',obj).subscribe(x =>{
    if(x !== ''){
...
SetDefaultFilters(Change){
  ...
  const obj = { 
    "CORAID":this.InventoryForm.value.InventoryControl,
  ...
  this.spinnerService.show();
  this.authService.AXELPostmethod('AXELData/AxelInvDataQueryDropdowns',obj).subscribe(x =>{
...
GridBind(){
  ...
  const obj = { 
    "CORAID":this.InventoryForm.value.InventoryControl,
    "STOCKTYPE":this.STORETYPE_Change,
  ...
  this.spinnerService.show();
  this.authService.AXELPostmethod('AXELData/AxelInventoryDataQuery',obj).subscribe(x =>{
